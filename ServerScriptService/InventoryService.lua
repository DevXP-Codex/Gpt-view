...
local function isNonStackable(itemId)
    return ItemCatalog.IsEquipment(itemId)
    ...
    for _, entry in ipairs(storage) do
        ...
        if itemId then
            if isNonStackable(itemId) then
                ...
                for _, normalizedEntry in ipairs(normalized) do
...
local function removeStorageItem(player, itemId, quantity)
    ...
    if getStorageItemQuantity(player, itemId) < quantity then
        return false
    end
