...
-- Centralized character portrait renderer.
-- IMPORTANT:
-- In Roblox Studio Lite, CharacterPreview is parented to MainMenu.
-- Do not assume it is a direct child of CharacterUI/PlayerScripts.
local function findCharacterPreviewModule()
    ...
    -- CharacterUI may be parented under MainMenu or another UI container.
...
if characterPreviewModule then
    ...
    warn(
        ...
        "Expected it to be parented to MainMenu."
    )
end
...
if CharacterPreview and CharacterPreview.Create then
    viewport = CharacterPreview.Create(viewportContainer, {
        AddCorner = addCorner,
    })
end

local viewportFallback = nil
...
local function clearCharacterViewport()
    ...
    if viewportFallback then
        viewportFallback.Visible = true
    end
end

local function renderCharacterViewport(viewportFrame, sourceCharacter)
    if not CharacterPreview or not viewportFrame or not sourceCharacter then
        clearCharacterViewport()
        return false
    end

    local ok, rendered = pcall(function()
        return CharacterPreview.Render(viewportFrame, sourceCharacter)
    end)

    if not ok then
        warn("[YAMA CharacterUI] CharacterPreview.Render failed:", rendered)
        clearCharacterViewport()
        return false
    end

    if viewportFallback then
        viewportFallback.Visible = not rendered
    end
...
local function render(data)
    ...
    if viewport and player.Character then
...
local function bindLiveCharacter(character, humanoid)
    ...
    -- CharacterSelected can arrive before the authoritative CharacterUI
    -- snapshot contains the runtime SP/XP values. Request the complete
    -- snapshot immediately after the character is fully ready.
    task.defer(function()
        if activeCharacterReady then
            remote:FireServer("GetData")
        end
