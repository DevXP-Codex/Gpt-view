...
-- CharacterPreview is a ModuleScript parented to MainMenu.
-- GitHub folder structure is NOT used for runtime lookup.
--------------------------------------------------

local function findCharacterPreviewModule()
    local function search(container)
        if not container then
            return nil
        end
        
        local direct = container:FindFirstChild("CharacterPreview")
        if direct and direct:IsA("ModuleScript") then
            return direct
        end
        
        local recursive = container:FindFirstChild("CharacterPreview", true)
    ...
    -- CharacterUI may live in PlayerScripts while MainMenu lives in PlayerGui.
    local module = search(script.Parent)
    if module then
        return module
    end
    
    module = search(script.Parent and script.Parent.Parent)
    if module then
        return module
    end
    
    module = search(playerGui)
    if module then
        return module
    end
    
    warn(
    "[YAMA CharacterUI] CharacterPreview ModuleScript was not found. " ..
    "Expected it under MainMenu."
    )
    
    return nil
end

local characterPreviewModule = findCharacterPreviewModule()
local CharacterPreview = nil

if characterPreviewModule then
    local ok, result = pcall(require, characterPreviewModule)
    
    if ok and type(result) == "table" then
        CharacterPreview = result
    else
        warn(
        "[YAMA CharacterUI] Failed to require CharacterPreview:",
        result
        )
    end
end
...
viewportContainer.Name = "CharacterRender2D"
viewportContainer.Position = UDim2.new(0, 5, 0, 24)
viewportContainer.Size = UDim2.new(1, -10, 1, -29)
viewportContainer.BackgroundColor3 = Color3.fromRGB(224, 211, 181)
viewportContainer.BorderSizePixel = 0
viewportContainer.ClipsDescendants = true
viewportContainer.Parent = overviewRight
addCorner(viewportContainer, 6)

local viewport = nil

if CharacterPreview and CharacterPreview.Create then
    local ok, result = pcall(function()
        return CharacterPreview.Create(viewportContainer, {
        AddCorner = addCorner,
        })
    end)
    ...
    if ok then
        ...
        warn(
        "[YAMA CharacterUI] CharacterPreview.Create failed:",
        result
        )
    end
end
...
if not viewport then
    ...
    "Fallback",
    "CHARACTER\nPREVIEW\nUNAVAILABLE",
    UDim2.new(1, -10, 0.55, 0),
    UDim2.new(0, 5, 0.23, 0),
    13,
    muted
    )
...
local function createSquareSlots(parent, slotNames, prefix)
    ...
    -- Seven equipment slots use a character-shaped arrangement.
    -- Visual slots use a separate 4x2 arrangement so nothing overlaps.
    local positions = {
    -- Equipment
    ...
    -- Visual
    Hair = UDim2.new(0.5, -88.5, 0, 2),
    Face = UDim2.new(0.5, -42.5, 0, 2),
    VisualHead = UDim2.new(0.5, 3.5, 0, 2),
    VisualBody = UDim2.new(0.5, 49.5, 0, 2),
    
    VisualLegs = UDim2.new(0.5, -65.5, 0, 49),
    VisualMainHand = UDim2.new(0.5, -19.5, 0, 49),
    VisualOffHand = UDim2.new(0.5, 26.5, 0, 49),
    VisualBack = UDim2.new(0.5, 72.5, 0, 49),
    }
    
    local visualPositionByName = {
    Hair = positions.Hair,
    Face = positions.Face,
    Head = positions.VisualHead,
    Body = positions.VisualBody,
    Legs = positions.VisualLegs,
    MainHand = positions.VisualMainHand,
    OffHand = positions.VisualOffHand,
    Back = positions.VisualBack,
    }
    
    local isVisual = string.find(prefix, "Visual_", 1, true) ~= nil
...
local function renderCharacterPreview(character)
    if not CharacterPreview or not viewport then
        return false
    end
    
    if not character or not character:IsA("Model") then
        if CharacterPreview.Clear then
    ...
    local ok, result = pcall(function()
        return CharacterPreview.Render(viewport, character)
    end)
    
    if not ok then
        warn(
        "[YAMA CharacterUI] CharacterPreview.Render failed:",
        result
        )
        return false
    end
...
local function render(data)
    ...
    if player.Character then
        renderCharacterPreview(player.Character)
    end
    ...
    if player.Character then
        renderCharacterPreview(player.Character)
    elseif CharacterPreview and viewport and CharacterPreview.Clear then
        CharacterPreview.Clear(viewport)
    end
...
local function hideCharacterUI()
    ...
    disconnectHealthConnection()
    if CharacterPreview and viewport and CharacterPreview.Clear then
        CharacterPreview.Clear(viewport)
    end
    closePanel(false)
    gui.Enabled = false
