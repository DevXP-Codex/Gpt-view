...
local function createAttributeRow(name, index)
    ...
    row.Name = name .. "Row"
    row.Size = UDim2.fromScale(0.55, 0.09)
    row.Position = UDim2.fromScale(0.08, 0.565 + ((index - 1) * 0.055))
...
local function optionIndex(category, id)
    for index, option in ipairs(customizationOptions[category] or {}) do
        if tostring(option.Id) == tostring(id) then return index end
    end
    return nil
end
...
local function updateCustomizationLabel(category)
    ...
    ref.value.Text = option and tostring(option.Name) or tostring(customization[category] or "Default")
end

local function requestCreationPreview()
    if not refs.characterRemote then return end
    previewRequestToken += 1
    local token = previewRequestToken
    task.delay(0.08, function()
        if token ~= previewRequestToken then return end
...
local function buildCustomizationRow(category, title, y)
    ...
    row.Size = UDim2.fromScale(0.55, 0.075)
    ...
    row.ZIndex = 35
    row.Parent = panel
    refs.addCorner(row, 7); refs.addStroke(row, colors.GoldDark, 1)
    
    local label = Instance.new("TextLabel")
    label.Size = UDim2.fromScale(0.22, 1); label.Position = UDim2.fromScale(0.03, 0)
    label.BackgroundTransparency = 1; label.Text = title; label.TextScaled = true
    label.TextColor3 = colors.TextLight; label.Font = Enum.Font.Fantasy; label.ZIndex = 36; label.Parent = row
    
    local previous = Instance.new("TextButton")
    previous.Size = UDim2.fromScale(0.12, 0.70); previous.Position = UDim2.fromScale(0.57, 0.15)
    previous.BackgroundColor3 = colors.WoodDark; previous.BorderSizePixel = 0; previous.Text = "‹"
    previous.TextScaled = true; previous.TextColor3 = colors.GoldBright; previous.Font = Enum.Font.Fantasy
    previous.AutoButtonColor = false; previous.ZIndex = 36; previous.Parent = row
    refs.addCorner(previous, 6); refs.addStroke(previous, colors.GoldDark, 1)
    
    local value = Instance.new("TextLabel")
    value.Size = UDim2.fromScale(0.26, 0.70); value.Position = UDim2.fromScale(0.70, 0.15)
    value.BackgroundTransparency = 1; value.Text = tostring(customization[category] or "Default"); value.TextScaled = true
    value.TextColor3 = colors.GoldBright; value.Font = Enum.Font.Fantasy; value.ZIndex = 36; value.Parent = row
    
    local nextButton = Instance.new("TextButton")
    nextButton.Size = UDim2.fromScale(0.12, 0.70); nextButton.Position = UDim2.fromScale(0.86, 0.15)
    nextButton.BackgroundColor3 = colors.WoodDark; nextButton.BorderSizePixel = 0; nextButton.Text = "›"
    nextButton.TextScaled = true; nextButton.TextColor3 = colors.GoldBright; nextButton.Font = Enum.Font.Fantasy
    nextButton.AutoButtonColor = false; nextButton.ZIndex = 36; nextButton.Parent = row
    refs.addCorner(nextButton, 6); refs.addStroke(nextButton, colors.GoldDark, 1)
    
    previous.Activated:Connect(function() setCustomization(category, -1) end)
        nextButton.Activated:Connect(function() setCustomization(category, 1) end)
            refs.customization = refs.customization or {}
            refs.customization[category] = {row=row, value=value, previous=previous, next=nextButton}
        end
        
        local function updateCustomizationOptions(data)
            if typeof(data) ~= "table" then return end
            for _, category in ipairs({"Skin", "Hair", "Face"}) do
                if typeof(data[category]) == "table" then customizationOptions[category] = data[category] end
            end
            if not optionIndex("Skin", customization.Skin) then customization.Skin = customizationOptions.Skin[1] and customizationOptions.Skin[1].Id or customization.Skin end
            if not optionIndex("Hair", customization.Hair) then customization.Hair = customizationOptions.Hair[1] and customizationOptions.Hair[1].Id or customization.Hair end
            if not optionIndex("Face", customization.Face) then customization.Face = customizationOptions.Face[1] and customizationOptions.Face[1].Id or customization.Face end
            for _, category in ipairs({"Skin", "Hair", "Face"}) do updateCustomizationLabel(category) end
        end
        ...
        function CharacterCreationUI.Initialize(config)
            ...
            buildCustomizationRow("Hair", "HAIR", 0.385)
            buildCustomizationRow("Face", "FACE", 0.465)
            ...
            portraitFrame.Size = UDim2.fromScale(0.30, 0.64)
            portraitFrame.Position = UDim2.fromScale(0.66, 0.21)
            ...
            portraitCaption.Size = UDim2.new(0.88, 0, 0.07, 0)
            portraitCaption.Position = UDim2.fromScale(0.06, 0.92)
            portraitCaption.BackgroundTransparency = 1
            portraitCaption.Text = "ADVENTURER"
            portraitCaption.TextScaled = true
            ...
            portraitFallback.Size = UDim2.new(0.8, 0, 0.35, 0)
            portraitFallback.Position = UDim2.fromScale(0.1, 0.32)
            ...
            attributeTitle.Position = UDim2.fromScale(0.08, 0.535)
