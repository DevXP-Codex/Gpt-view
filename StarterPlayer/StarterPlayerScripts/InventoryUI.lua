...
equipButton.Size = UDim2.fromScale(0.86, 0.085)
equipButton.Position = UDim2.fromScale(0.07, 0.625)
equipButton.BackgroundColor3 = COLORS.Wood
equipButton.BorderSizePixel = 0
equipButton.Text = "EQUIP"
equipButton.TextColor3 = COLORS.GoldBright
equipButton.TextScaled = true
equipButton.Font = Enum.Font.Fantasy
equipButton.AutoButtonColor = false
equipButton.ZIndex = 5
equipButton.Parent = details
addCorner(equipButton, 8)
addStroke(equipButton, COLORS.Gold, 2)
addGradient(equipButton, COLORS.Wood, COLORS.WoodDark, 90)
UIAudio.BindButton(equipButton, {Click = "Confirm"})

local storageButton = Instance.new("TextButton")
storageButton.Name = "MoveToStorage"
storageButton.Size = UDim2.fromScale(0.86, 0.085)
storageButton.Position = UDim2.fromScale(0.07, 0.72)
...
dropButton.Size = UDim2.fromScale(0.86, 0.085)
dropButton.Position = UDim2.fromScale(0.07, 0.815)
...
storageOpenButton.Size = UDim2.fromScale(0.86, 0.065)
storageOpenButton.Position = UDim2.fromScale(0.07, 0.91)
...
local function isEquipableEntry(entry)
    if not entry then
        return false
    end
    
    local data = entry.Data or {}
    local itemType = tostring(data.Type or "")
...
local function updateActionButtons()
    ...
    setActionEnabled(equipButton, enabled and isEquipableEntry(entry))
...
equipButton.Activated:Connect(function()
    ...
    if not entry or not isEquipableEntry(entry) then
        UIAudio.Play("Error")
        return
    end
...
CharacterRemote.OnClientEvent:Connect(function(action, data)
    ...
    elseif action == "InventoryOpened" then
        ...
        if type(data) == "table" then
            -- Compatibility with the current GameServer payload:
            -- it sends the raw inventory table directly.
            -- Also keep support for the newer InventoryService state format.
    ...
    elseif action == "ItemEquipped" then
        UIAudio.Play("Confirm")
        statusLabel.Text = "Item equipped."
        updateActionButtons()
        
    elseif action == "ItemEquipFailed" then
        UIAudio.Play("Error")
        statusLabel.Text = "Unable to equip this item."
